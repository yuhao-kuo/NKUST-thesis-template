...
\setboolean{isthesistitleexternal}{false}       % 封面, true: LaTeX生成, false: 匯入外部pdf
\def\externalmaintitle{Externals/maintitle}     % 外部載入封面位置, 設定由 LaTeX 生成時, 此欄位請忽視

\setboolean{isthesisbooknameexternal}{false}    % 書名頁, true: LaTeX生成, false: 匯入外部pdf
...
\provideboolean{thesissign}
\provideboolean{thesisphdrecommand}

% 博碩士論文授權書
\def\thesispowerofattorney{Externals/powerofattorney.pdf}   % 博碩士論文授權書位置
\setboolean{thesisauht}{true}                               % 博碩士論文授權書, true: 載入, false: 不載入

% 論文口試委員會審定書
\def\thesisvalidation{Externals/sign.pdf}                   % 論文口試委員會審定書位置
\setboolean{thesissign}{true}                               % 論文口試委員會審定書, true: 載入, false: 不載入
